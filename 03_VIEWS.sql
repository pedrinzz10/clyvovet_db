...
-- Faturamento consolidado por clinica
-- ------------------------------------------------------------
...
-- Historico consolidado por animal: eventos, gastos e tutor
-- ------------------------------------------------------------
...
SELECT
    ...
    MIN(ec.data)                                                                 AS primeiro_evento,
    MAX(ec.data)                                                                 AS ultimo_evento,
...
LEFT JOIN pagamento       p  ON p.evento_clinico_id = ec.id
GROUP BY a.id, a.nome, a.especie, a.raca, a.porte, a.sexo, a.data_nascimento,
         t.nome, t.telefone, t.email;

-- ------------------------------------------------------------
-- VIEW 4: vw_agenda_veterinario
-- Proximos eventos por veterinario com dados do animal e tutor
-- ------------------------------------------------------------
...
JOIN animal         a  ON a.id  = ec.animal_id
JOIN tutor          t  ON t.id  = a.tutor_id
WHERE ec.data >= TRUNC(SYSDATE)
ORDER BY v.nome, ec.data, ec.hora;
